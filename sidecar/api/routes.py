...
async def explain_report(request: Request, body: ExplainRequest = Body(...)):
    ...
    if body.quick_normal:
        ...
        pet_kwargs = {"pet_rules_v2": True} if body.pet_rules_v2 and hasattr(handler, '_inject_cfc') else {}
        prompt_context = handler.get_prompt_context(extraction_result, **pet_kwargs) if handler else {}
...
async def _explain_stream_gen(explain_request: ExplainRequest, user_id: str | None = None):
    ...
    try:
        ...
        prompt_engine = PromptEngine()
        pet_kwargs = {"pet_rules_v2": True} if explain_request.pet_rules_v2 and hasattr(handler, '_inject_cfc') else {}
        prompt_context = handler.get_prompt_context(extraction_result, **pet_kwargs) if handler else {}
        if not handler:
            prompt_context["test_type_hint"] = test_type
        if settings.specialty and "specialty" not in prompt_context:
