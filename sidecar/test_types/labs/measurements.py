...
MEASUREMENT_DEFS: list[MeasurementDef] = [
    ...
    MeasurementDef(
        ...
        patterns=[
            rf"(?i)(?<![A-Za-z])(?:total\s+)?cholesterol{_SEP}{_NUM}\s*(?:mg\/dL)?",
            rf"(?i)cholesterol[,\s]+total{_SEP}{_NUM}\s*(?:mg\/dL)?",
        ],
        value_min=50.0,
        value_max=500.0,
    ),
    ...
    MeasurementDef(
        ...
        patterns=[
            rf"(?i)(?<![a-zA-Z])(?:direct\s+)?LDL(?:\s+cholesterol)?(?:\s+(?:calc(?:ulated)?|direct))?(?:\s+chol\.?|-C)?{_SEP}{_NUM}\s*(?:mg\/dL)?",
        ],
        value_min=10.0,
        value_max=400.0,
    ),
    ...
    MeasurementDef(
        ...
        abbreviation="NON_HDL",
        unit="mg/dL",
        table_aliases=["non-hdl", "non-hdl cholesterol", "non hdl",
                        "non hdl cholesterol", "non-hdl-c"],
        patterns=[
            rf"(?i)non[\s-]?HDL(?:\s+cholesterol|\s+chol\.?|-C)?{_SEP}{_NUM}\s*(?:mg\/dL)?",
        ],
        value_min=10.0,
        value_max=400.0,
    ),
    MeasurementDef(
        ...
        abbreviation="LPA",
        unit="nmol/L",
        table_aliases=["lipoprotein(a)", "lipoprotein a", "lp(a)", "lpa",
                        "lp a", "lipoprotein (a)", "lipoprotein (a) (nmol/l)",
                        "lipoprotein(a) (nmol/l)", "lp(a) (nmol/l)"],
        patterns=[
            rf"(?i)(?:lipoprotein\s*\(?\s*a\s*\)?|Lp\s*\(?\s*a\s*\)?)\s*(?:\(?\s*(?:nmol\/L|mg\/dL)\s*\)?)?\s*{_NUM}",
        ],
        value_min=0.0,
        value_max=500.0,
    ),
    MeasurementDef(
        ...
        abbreviation="APOB",
        unit="mg/dL",
        table_aliases=["apolipoprotein b", "apo b", "apob", "apo-b",
                        "apolipoprotein b-100", "apolipoprotein b (mg/dl)"],
        patterns=[
            rf"(?i)(?:apolipoprotein\s*B(?:-?100)?|apo[\s-]?B)\s*(?:\(?\s*mg\/dL\s*\)?)?\s*{_NUM}",
        ],
        value_min=10.0,
        value_max=300.0,
    ),
    ...
    MeasurementDef(
        ...
        abbreviation="LDL_P",
        unit="nmol/L",
        table_aliases=["ldl-p", "ldl particle number", "ldl particles",
                        "ldl particle count", "ldl-p (nmol/l)"],
        patterns=[
            rf"(?i)LDL[\s-]?(?:P|particle(?:s|\s+number|\s+count)?){_SEP}{_NUM}\s*(?:nmol\/L)?",
        ],
        value_min=200.0,
        value_max=3000.0,
    ),
    MeasurementDef(
        name="Small LDL Particle Number",
        abbreviation="SMALL_LDL_P",
        unit="nmol/L",
        table_aliases=["small ldl-p", "small ldl particles",
                        "small ldl particle number", "small ldl-p (nmol/l)"],
        patterns=[
            rf"(?i)small\s+LDL[\s-]?(?:P|particle(?:s|\s+number)?){_SEP}{_NUM}\s*(?:nmol\/L)?",
        ],
        value_min=0.0,
        value_max=2000.0,
    ),
    MeasurementDef(
        ...
        abbreviation="LDL_SIZE",
        unit="nm",
        table_aliases=["ldl size", "ldl particle size", "peak ldl diameter",
                        "ldl peak size", "ldl size (nm)"],
        patterns=[
            rf"(?i)(?:LDL\s+(?:particle\s+)?size|peak\s+LDL\s+diameter){_SEP}{_NUM}\s*(?:nm)?",
        ],
        value_min=15.0,
        value_max=25.0,
    ),
    MeasurementDef(
        name="Large HDL Particle Number",
        abbreviation="LARGE_HDL_P",
        unit="umol/L",
        table_aliases=["large hdl-p", "large hdl particles",
                        "hdl large particles", "large hdl-p (umol/l)"],
        patterns=[
            rf"(?i)(?:large\s+HDL[\s-]?P|HDL\s+large\s+particle(?:s)?){_SEP}{_NUM}\s*(?:umol\/L)?",
        ],
        value_min=0.0,
        value_max=30.0,
    ),
    MeasurementDef(
        name="Large VLDL Particle Number",
        abbreviation="LARGE_VLDL_P",
        unit="nmol/L",
        table_aliases=["large vldl-p", "large vldl particles",
                        "vldl large particles", "large vldl-p (nmol/l)"],
        patterns=[
            rf"(?i)(?:large\s+VLDL[\s-]?P|VLDL\s+large\s+particle(?:s)?){_SEP}{_NUM}\s*(?:nmol\/L)?",
        ],
        value_min=0.0,
        value_max=50.0,
    ),
    MeasurementDef(
        ...
        abbreviation="LP_IR",
        unit="",
        table_aliases=["lp-ir", "lp-ir score", "lipoprotein insulin resistance",
                        "lp-ir score (<=45)"],
        patterns=[
            rf"(?i)LP[\s-]?IR(?:\s+score)?{_SEP}{_NUM}",
            rf"(?i)lipoprotein\s+insulin\s+resistance{_SEP}{_NUM}",
        ],
        value_min=0.0,
        value_max=100.0,
    ),
    MeasurementDef(
        ...
        abbreviation="SDLDL",
        unit="mg/dL",
        table_aliases=["sdldl", "sd-ldl", "small dense ldl",
                        "small dense ldl cholesterol", "sdldl-c"],
        patterns=[
            rf"(?i)(?:sd[\s-]?LDL|small\s+dense\s+LDL)(?:[\s-]?(?:cholesterol|chol\.?|C))?{_SEP}{_NUM}\s*(?:mg\/dL)?",
        ],
        value_min=1.0,
        value_max=100.0,
    ),
    MeasurementDef(
        name="Lp-PLA2",
        abbreviation="LP_PLA2",
        unit="ng/mL",
        table_aliases=["lp-pla2", "plac test", "plac",
                        "lipoprotein-associated phospholipase a2",
                        "lp-pla2 activity"],
        patterns=[
            rf"(?i)Lp[\s-]?PLA2(?:\s+activity)?{_SEP}{_NUM}\s*(?:ng\/mL|U\/L)?",
            rf"(?i)PLAC(?:\s+test)?{_SEP}{_NUM}\s*(?:ng\/mL)?",
            rf"(?i)lipoprotein[\s-]?associated\s+phospholipase{_SEP}{_NUM}",
        ],
        value_min=0.0,
        value_max=1000.0,
    ),
    MeasurementDef(
        ...
        abbreviation="HSCRP",
        unit="mg/L",
        table_aliases=["hs-crp", "hscrp", "high-sensitivity crp",
                        "high sensitivity c-reactive protein",
                        "c-reactive protein, cardiac",
                        "crp, high sensitivity", "cardio crp"],
        patterns=[
            rf"(?i)(?:hs[\s-]?CRP|high[\s-]?sensitivity\s+(?:C[\s-]?reactive\s+protein|CRP)){_SEP}{_NUM}\s*(?:mg\/L)?",
            rf"(?i)C[\s-]?reactive\s+protein[,\s]+(?:cardiac|high\s+sensitivity){_SEP}{_NUM}\s*(?:mg\/L)?",
            rf"(?i)cardio\s+CRP{_SEP}{_NUM}\s*(?:mg\/L)?",
        ],
        value_min=0.0,
        value_max=50.0,
    ),
    MeasurementDef(
        ...
        abbreviation="HCY",
        unit="umol/L",
        table_aliases=["homocysteine", "hcy", "homocysteine, plasma",
                        "homocysteine, serum"],
        patterns=[
            rf"(?i)homocysteine{_SEP}{_NUM}\s*(?:umol\/L|mcmol\/L)?",
            rf"(?i)\bHcy\b{_SEP}{_NUM}\s*(?:umol\/L)?",
        ],
        value_min=1.0,
        value_max=100.0,
    ),
    MeasurementDef(
        ...
        abbreviation="OMEGA3",
        unit="%",
        table_aliases=["omega-3 index", "omega 3 index", "omega-3 index (%)",
                        "omegacheck"],
        patterns=[
            rf"(?i)omega[\s-]?3\s+index{_SEP}{_NUM}\s*%?",
        ],
        value_min=0.5,
        value_max=20.0,
    ),
    MeasurementDef(
        ...
        unit="uIU/mL",
        table_aliases=["insulin", "fasting insulin", "insulin, fasting",
                        "serum insulin"],
        patterns=[
            rf"(?i)(?:fasting\s+)?insulin{_SEP}{_NUM}\s*(?:uIU\/mL|mIU\/L)?",
        ],
        value_min=0.5,
        value_max=200.0,
    ),
